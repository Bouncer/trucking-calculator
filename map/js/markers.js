const business_icons = {
    ...
    "Trucking": `${custom_emoji_folder}1F69B.png`,
}
...
fetch(base_folder + "data/trucking.json").then(res=>res.json()).then(res=>{
    for (const key in res) {
        const marker = L.marker(
            [res[key].x, res[key].y],
            { icon:  create_data_marker_icon("Trucking") } 
        ).bindPopup(`<div class="popup-header blue-bg">${key}</div>
        Type: Self Storage Unit<hr>`);

        static_markers_list.trucking.markers.push(marker);
        if(options.markers["trucking"]) window.mainMap.addLayer(marker);
    };
});
